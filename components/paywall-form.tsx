...
export function PaywallForm({ config }: PaywallFormProps) {
    const [status, setStatus] = useState<"idle" | "submitting" | "payment_required" | "paying" | "success">("idle");
    ...
    const handleSubmit = async (e?: React.FormEvent, paymentProof?: string) => {
        ...
        if (status === "submitting" || (status === "paying" && !paymentProof)) return;
    ...
    const handlePay = async () => {
        ...
        if (!paymentRequirements) {
            alert("Error: No payment requirements found. Please try submitting again first.");
            return;
        }
        ...
        try {
            ...
            const selectedRequirement = selectPaymentRequirements(paymentRequirements, APP_CONFIG.NETWORK, "exact");

            if (!selectedRequirement) {
                console.error("Requirements:", paymentRequirements);
                throw new Error(`No matching payment requirement found for network: ${APP_CONFIG.NETWORK}`);
            }
            ...
            console.log("Payment Header Created (Base64):", paymentHeader.slice(0, 20) + "...");
    ...
    if (status === "success") {
        return (
            <div className="flex flex-col items-center justify-center p-8 bg-green-50 rounded-xl border border-green-200">
                ...
                <p className="text-green-600 mt-2 text-center">Your message has been delivered to the creator.</p>
                <button onClick={() => window.location.reload()} className="mt-6 px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700">
                    Send Another
                </button>
            </div>
        );
    ...
    return (
        <div className="relative w-full max-w-md mx-auto">
            ...
            <div className={cn(
                ...
                (status === "payment_required" || status === "paying") ? "blur-sm pointer-events-none opacity-50" : ""
            )}>
                ...
                {(status === "payment_required" || status === "paying") && (
                    ...
                    >
                        <div className="bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-gray-200 w-full max-w-sm text-center">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 mb-2">Gatekeeper Active</h3>
                            <p className="text-gray-500 text-sm mb-6">
                                This creator requires a small payment to verify serious inquiries.
                            </p>

                            <div className="bg-gray-50 rounded-xl p-4 mb-6">
                                <div className="text-sm text-gray-500 mb-1">Pay Amount</div>
                                <div className="text-3xl font-bold text-gray-900">{config.price} {config.currency}</div>
                            </div>
